---
title: "plant.rel.HJA.fungi.analysis.rmd"
author: "Kyle Gervers"
...
needle <- function(x) {
  ...
  plant.hja.phy <- subset_taxa(x, Genus == "g__Pseudotsuga") %>% subset_samples(Control == "No" & Organ == "Needle") %>% subset_samples(Tree == "T1" | Tree == "T3" | Tree == "T5" | Tree == "T6") %>% subset_samples(sampleID != "P2_E_8")
  
  plant.hja.phy %<>% prune_samples(keep.reads,.)
  
  plant.hja.sums <- sample_sums(plant.hja.phy)
  
  otu.table <- sapply(data.frame(otu_table(needle.fungi.hja.phy)),function(x) x/plant.hja.sums)
  
  needle.fungi.hja.phy <- transform_sample_counts(needle.fungi.hja.phy, function(x) {sqrt(x)})
  
  otu_table(needle.fungi.hja.phy) <- otu_table(otu.table, taxa_are_rows = FALSE)
  ...
  write.csv(needle.fungi.hja.otu, file = "4.plant.needle.fungi.hja.otu.csv")
  
  write.csv(needle.fungi.hja.meta, file = "4.plant.needle.fungi.hja.meta.csv")
...
twig <- function(x) {
  ...
  twig.fungi.hja.phy %<>% prune_samples(keep.reads,.) %>% filter_taxa(function(x) {sum(x > 25) > (0.01*length(x))}, TRUE)
  
  plant.hja.phy <- subset_taxa(x, Genus == "g__Pseudotsuga") %>% subset_samples(Control == "No" & Organ == "Twig") %>% subset_samples(Tree == "T1" | Tree == "T3" | Tree == "T5" | Tree == "T6")
  
  plant.hja.phy %<>% prune_samples(keep.reads,.)
  
  plant.hja.sums <- sample_sums(plant.hja.phy)
  
  otu.table <- sapply(data.frame(otu_table(twig.fungi.hja.phy)),function(x) x/plant.hja.sums)
  
  twig.fungi.hja.phy <- transform_sample_counts(twig.fungi.hja.phy, function(x) {sqrt(x)})
  
  otu_table(twig.fungi.hja.phy) <- otu_table(otu.table, taxa_are_rows = FALSE)
  
  twig.fungi.hja.otu <- otu_table(twig.fungi.hja.phy) %>% data.frame()
  
  twig.fungi.hja.meta <- sample_data(twig.fungi.hja.phy) %>% data.frame()
  
  write.csv(twig.fungi.hja.otu, file = "4.plant.twig.fungi.hja.otu.csv")
  
  write.csv(twig.fungi.hja.meta, file = "4.plant.twig.fungi.hja.meta.csv")
...
ord.needle <- function(x){
  ...
  needle.fungi.hja.ord <- ordinate(needle.fungi.hja.phy, "NMDS", "bray" , trymax = 75)

  #height.needle.fungi.hja.ord.plot <- plot_ordination(needle.fungi.hja.phy, needle.fungi.hja.ord, type = "samples", color = "Ht")
  age.needle.fungi.hja.ord.plot <- plot_ordination(needle.fungi.hja.phy, needle.fungi.hja.ord, type = "samples", shape = "Age", color = "Age")
  #canopy.needle.fungi.hja.ord.plot <- plot_ordination(needle.fungi.hja.phy, needle.fungi.hja.ord, type = "samples", shape ="CanopyLevel", color = "CanopyLevel")
  ...
  #relht.needle.fungi.hja.ord.plot <- plot_ordination(needle.fungi.hja.phy, needle.fungi.hja.ord, type = "samples", color = "RelHeight")
  # htminveg.needle.fungi.hja.ord.plot <- plot_ordination(x, needle.fungi.hja.ord, type = "samples", color = "HtMinusVeg")
  #htz.needle.fungi.hja.ord.plot <- plot_ordination(x, needle.fungi.hja.ord, type = "samples", color = "HtPlusZ")
  
  #htz.needle.fungi.hja.ord.plot + geom_point(size=5) + ggtitle("Needle samples by elevation")# + facet_wrap(~Age, 3)
  #htminveg.needle.fungi.hja.ord.plot + geom_point(size=5) + scale_fill_gradient2(midpoint = 3) + ggtitle("Needle")
  #relht.needle.fungi.hja.ord.plot + geom_point(size=5) + scale_fill_gradient2(midpoint = 3) + ggtitle("Needle samples by relative height in tree") + facet_wrap(~Tree, 3)
  ...
  #tree.needle.fungi.hja.ord.plot + geom_point(size=5) + scale_color_colorblind() + ggtitle("Needle samples by tree")# + facet_wrap(~Tree, 3)
  #height.needle.fungi.hja.ord.plot + geom_point(size=5) + scale_fill_gradient2(midpoint = 3) + ggtitle("Needle") + facet_wrap(~Tree, 3)
  #canopy.needle.fungi.hja.ord.plot + geom_point(size=5) + ggtitle("Needle samples by position in the canopy") + facet_wrap(~Tree, 3)
  age.needle.fungi.hja.ord.plot + geom_point(size=5) + ggtitle("Needle samples by age")
...
ord.twig <- function(x){
  ...
  twig.fungi.hja.ord <- ordinate(x, "NMDS", "bray", trymax = 75)

  #height.twig.fungi.hja.ord.plot <- plot_ordination(x, twig.fungi.hja.ord, type = "samples", color = "Height")
  #htz.twig.fungi.hja.ord.plot <- plot_ordination(x, twig.fungi.hja.ord, type = "samples", color = "HtPlusZ")
  ...
  #htz.twig.fungi.hja.ord.plot + geom_point(size=5) + scale_fill_gradient2(midpoint = 3) + ggtitle("Twig samples by height") 
  #htminveg.twig.fungi.hja.ord.plot + geom_point(size=5) + scale_fill_gradient2(midpoint = 3) + ggtitle("Twig")
  #relht.twig.fungi.hja.ord.plot + geom_point(size=5) + scale_fill_gradient2(midpoint = 3) + ggtitle("Twig samples by height") 
  ...
  tree.twig.fungi.hja.ord.plot + geom_point(size=5) + scale_color_colorblind() + ggtitle("Twig samples by tree")
...
#adonis(needle.dist~RelHeight,data = needle.fungi.hja.meta, strata = needle.fungi.hja.meta$Tree)

#twig.dist <- phyloseq::distance(twig.fungi.hja.phy,"bray")
#adonis(twig.dist~Tree, data = twig.fungi.hja.meta)
